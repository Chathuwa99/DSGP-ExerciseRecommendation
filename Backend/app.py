...
def predict():
    try:
        ...
        # Get exercise recommendations based on the predicted class
        ...
        if not exercise_data.empty:
            ...
            bmi_case = "Unknown"
            exercise = "General workout" if preference == "gym" else "Bodyweight exercises"

        return jsonify({
            "bmi": round(bmi, 2),         # Include calculated BMI
            "bmi_case": bmi_case,
            "exercise": exercise           # Return exercises based on preference
        })
